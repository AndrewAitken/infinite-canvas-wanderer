...
const InfiniteCanvas: React.FC = () => {
  ...
  return <>
      <div ref={canvasRef} onMouseDown={handleMouseDown} onTouchStart={handleTouchStart} style={{
      cursor: isDragging ? 'grabbing' : 'grab'
        ...
        transition: isDragging ? 'none' : 'transform 0.3s ease-out'
          ...
          {visibleItems.map(item => <CoverSquare key={`${item.gridX}-${item.gridY}`} x={item.x} y={item.y} gridX={item.gridX} gridY={item.gridY} canvasSize={canvasSize} offset={offset} onAlbumClick={handleAlbumClick} />)}
        </div>
      </div>

      <AlbumDetailPanel album={selectedAlbum} isOpen={isPanelOpen} onClose={handlePanelClose} onNext={handleNextAlbum} onPrevious={handlePreviousAlbum} currentIndex={currentAlbumIndex} totalCount={allAlbums.length} />
    </>;
};
